...
class Platform {
  constructor(newPlatBottom) {
    ...
    if (
      random < 100 * difficultymultiply &&
      random >= 50 * difficultymultiply
    ) {
...
$(document).on('click', '.doodlejump-backbutton', function () {
  $('.doodlejump-rankscreen').fadeOut(appOpenspeed);
  $('.doodlejump-settingsscreen').fadeOut(appOpenspeed);
  $('.doodlejump-startscreen').fadeIn(appOpenspeed);
  ShowBackButton(-6);
  $('.doodlejump-homebutton').fadeOut(appOpenspeed);
  $('#doodlejump-gameover').fadeOut(appOpenspeed);
});
...
$(document).on('click', '.doodlejump-ranksbutton', function () {
  $('.doodlejump-rankscreen').fadeIn(appOpenspeed);
  $('.doodlejump-startscreen').fadeOut(appOpenspeed);
  ShowBackButton(1.5);
});

$(document).on('click', '.doodlejump-settings', function () {
  $('.doodlejump-settingsscreen').fadeIn(appOpenspeed);
  $('.doodlejump-startscreen').fadeOut(appOpenspeed);
  ShowBackButton(1.5);
});
...
function endscreen(score) {
  ...
  $('.doodlejump-endscreen').fadeIn(appOpenspeed);
  ...
  $('.doodlejump-startbutton').fadeIn(appOpenspeed);
}

function hideEndscreen() {
  $('.doodlejump-endscreen').fadeOut(appOpenspeed);
  $('.doodlejump-homebutton').fadeOut(appOpenspeed);
  $('.doodlejump-startbutton').fadeOut(appOpenspeed);
}
...
function moveplatform() {
  platforms.forEach((platform) => {
    if (platform.moveplatform == true) {
      if (platform.rightside == true)
        platform.left -= 0.05 * difficultymultiply;
      if (platform.rightside == false)
        platform.left += 0.05 * difficultymultiply;
