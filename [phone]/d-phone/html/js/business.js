...
setTimeout(() => {
  $('#business-alerts').hide(0);
}, 1000);

$(document).keyup(function (data) {
  if (data.which == 13) {
    if (
      business.tabopen == '#business-home-teamchat' &&
      ...
      (business.tabopen == '#business-home-dispatch' &&
        Main.openapp == 'business') ||
      (business.tabopen == '#business-home-dispatch' &&
...
$(function () {
  window.addEventListener('message', function (event) {
    ...
    } else if (v.app == 'business' && v.task == 'hideapp') {
      $('#businessicon').hide();
      $('#jobmailiicon').hide();
    }
    ...
    if (v.app == 'business' && v.task == 'setjob') {
      business.job = v.job;
      business.joblabel = v.label;
      SetTheme(v.job);
    }
...
business.show = function () {
  if (business.loaded == false) {
    sendData('business:load');
    Home();
    return;
  }
...
business.LoadHome = function (html, motd) {
  ...
  BottomSlideDown('#business-settings-input-area', appOpenspeed, -70);
  business.SortHome();
  business.loaded = true;
};

business.setUnread = function (amount) {
  let unreadhtml = amount;
  ...
  if (amount > 0) {
    $('#business-alerts').show(0);
  } else {
    $('#business-alerts').hide(0);
  }

  $('#business-alerts').html(unreadhtml);
};

business.SortHome = function () {
  var $wrapper = $('.phone-business-member-rank');
  ...
  $wrapper
    .find('.business-home-element')
    .sort(function (a, b) {
      return -a.getAttribute('data-grade') + +b.getAttribute('data-grade');
    })
    .appendTo($wrapper);
};
...
$(document).on('click', '#business-teamchat-selection-cancel', function () {
  BottomSlideDown('#business-teamchat-selection-inputarea', appOpenspeed, -70);
...
$(document).on(
  ...
  function () {
    BottomSlideUp('#business-teamchat-inputimageurl-area', appOpenspeed, 5);
    $('#business-teamchat-selection-inputarea').fadeOut(appOpenspeed);
  }
);

$(document).on('click', '#business-teamchat-urlinput-cancel', function () {
  BottomSlideDown('#business-teamchat-inputimageurl-area', appOpenspeed, -70);
...
$(document).on(
  'click',
  '#business-teamchat-selection-inputarea-photo',
  function () {
...
business.teamchat.AddPhotoNew = function () {
  ...
  if (
    $('#business-teamchat-selection-inputarea-photo').hasClass('new') == false
  ) {
...
business.teamchat.AddImageNew = function () {
  ...
  if (
    $('#business-teamchat-selection-inputarea-image').hasClass('new') == false
  ) {
...
business.teamchat.Load = function (html) {
  ...
  business.teamchat.newMessageHeader = new NewMessageHeader(
    'business-teamchat'
  );
...
$(document).on(
  'click',
  '#business-teamchat-selection-inputarea-gps',
  function () {
    sendData('business:teamchat:sendgps', {});
    BottomSlideDown(
      '#business-teamchat-selection-inputarea',
      appOpenspeed,
      -70
    );
  }
...
business.teamchat.AddMessage = function (html, me) {
  ...
  if (me == 'business-message-me') {
    ...
    business.teamchat.newMessageHeader.show();
  }
};
...
$(document).on('click', '#business-settings-motd', function () {
  BottomSlideUp('#business-settings-input-area', appOpenspeed, 5);
});
...
function setjobnumber() {
  if (
    document.getElementById('business-settings-setjobnumber-switch').checked
  ) {
...
$(document).on('click', '#business-member-managerank', function () {
  BottomSlideUp('#business-member-managerank-area', appOpenspeed, 5);
});

$(document).on('click', '#business-member-managerank-area-cancel', function () {
  BottomSlideDown('#business-member-managerank-area', appOpenspeed, -70);
});
...
business.dispatch.Load = function (html) {
  ...
  var array = [
    $('#business-dispatch-overview-container-accepted'),
    $('#business-dispatch-overview-container-accepted_others'),
    $('#business-dispatch-overview-container-open'),
  ];

  $.each(array, function (indexInArray, value) {
    value
      .find('.business-dispatches-overview-element')
      .sort(function (a, b) {
        return -a.getAttribute('data-place') + +b.getAttribute('data-place');
      })
      .appendTo(value);
  ...
  business.dispatch.newMessageHeader = new NewMessageHeader(
    'business-dispatch'
  );
};
...
$(document).on('click', '#business-dispatch-return', function () {
  ...
  //     "margin-left": "-100%"
  // })
  // BottomSlideDown('#business-dispatch-accept-area', appOpenspeed, -70);

  businessfooteropen('#business-dispatch-overview');

  sendData('business:dispatch:closedispatch');
});
...
business.dispatch.LoadDispatch = function (html, accepted) {
  ...
  business.dispatch.accepted = accepted;

  if (accepted == 0 && business.dispatch.hasseennotify == 0) {
    uinotify.show(
      'business',
      getLocal('dispatches'),
      getLocal('DispatchInfo'),
      5000,
      false,
      true
    );

    business.dispatch.hasseennotify = 1;
  }
};
...
business.dispatch.AddMessage = function (html, me) {
  ...
  if (me == 'business-message-me') {
    ...
    business.dispatch.newMessageHeader.show();
  }
};
...
$(document).on(
  'click',
  '#business-dispatch-selection-inputarea-gps',
  function () {
    sendData('business:dispatch:sendgps', {});
    BottomSlideDown(
      '#business-dispatch-selection-inputarea',
      appOpenspeed,
      -70
    );
  }
...
$(document).on('click', '#business-dispatch-add', function () {
  Inputarea.message.show(appOpenspeed, 'dispatch');
  $('.censorlayer').fadeIn(appOpenspeed);
...
$(document).on('click', '#business-dispatch-selection-cancel', function () {
  BottomSlideDown('#business-dispatch-selection-inputarea', appOpenspeed, -70);
  $('.censorlayer').fadeOut(appOpenspeed);
});

$(document).on(
  'click',
  '#business-dispatch-selection-inputarea-image',
  function () {
    BottomSlideUp('#business-dispatch-inputimageurl-area', appOpenspeed, 5);
    $('#business-dispatch-selection-inputarea').fadeOut(appOpenspeed);
  }
);

$(document).on('click', '#business-dispatch-urlinput-cancel', function () {
  BottomSlideDown('#business-dispatch-inputimageurl-area', appOpenspeed, -70);
...
function CheckStringLength(string) {
  if (string.length > 255) {
    ...
    return false;
  } else if (string.length === 0 && business.message.addedimage == null && messages.chat.addedimages.length == 0) {
...
$(document).on('click', '#business-home-member-add', function () {
  BottomSlideUp('#business-home-add-id', appOpenspeed, 5);
});

$(document).on('click', '#business-home-add-id-cancel', function () {
  BottomSlideDown('#business-home-add-id', appOpenspeed, -70);
});
...
$(document).on('click', '#business-home-add-id-submit', function () {
  ...
  BottomSlideUp('#business-home-add-rank', appOpenspeed, 5);
...
$(document).on('click', '#business-home-add-rank-submit', function () {
  ...
  BottomSlideDown('#business-home-add-rank', appOpenspeed, -70);
