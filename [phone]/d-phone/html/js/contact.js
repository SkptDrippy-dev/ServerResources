...
$(document).on('click', '#phone-app-contact-new-savebutton', function () {
  ...
  if (
    ...
    name.includes('<audio') == true ||
    name.includes('</audio') == true ||
    name.includes('html') == true ||
    name.includes('iframe') == true ||
    name.includes('src') == true ||
    name.includes('div') == true ||
    name.includes('div') == true ||
    name.includes('mp4') == true ||
    name.includes('<') == true ||
    name.includes('>') == true
  ) {
    ...
    if (
      number.indexOf('script') > -1 ||
      number.indexOf('http') > -1 ||
      number.indexOf('www') > -1 ||
      number.includes('<audio') == true ||
      number.includes('</audio') == true
    ) {
      sendData('notification', { text: 'Nice try d:^)', length: 5000 });
    } else {
...
$(document).on('click', '#phone-app-contact-page-call', function () {
  call.new(selectednumber, selectedname, false, true);
});

Startcall = function (number, label) {
  call.new(number, label, false, false);
};

//
$(document).on('click', '#phone-app-contact-load', function () {
  sendData('HardLoadContacts');
});
